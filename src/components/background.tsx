...
export default function Background() {
  ...
  const rngs = React.useMemo(() => {
    ...
    for (let y = -square / 2; y < height + square; y += square)
      rngs.push(new Random(MersenneTwister19937.seedWithArray([...entropy, i++])))
  ...
  const lines = React.useMemo(() => {
    ...
    for (let j = 1; j < dots.length; ++j) {
      ...
      for (let i = 0; i < j; ++i) {
        ...
        const ratio = (Math.pow(from.x - to.x, 2) + Math.pow(from.y - to.y, 2)) / Math.pow(square * reach, 2)
        if (ratio > 1)
          continue
    ...
    for (let dot of dots) {
      if (dot.lines.length <= 5)
        continue

      dot.lines.sort((a, b) => {
        if (a.from === dot && b.from !== dot)
          return 1
        else if (a.from !== dot && b.from === dot)
          return -1
  ...
  return (
    <svg className={styles.background} viewBox={`0 0 ${width} ${height}`} xmlns="http://www.w3.org/2000/svg">
      {dots.map((dot, i) =>
        <circle key={i} cx={dot.x} cy={dot.y} r={dot.size} fill="currentColor" />
      )}

      {lines.map(({ from, to }, i) =>
        <line key={i} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke="currentColor" strokeWidth={1} />
      )}
    </svg>
  )
}
